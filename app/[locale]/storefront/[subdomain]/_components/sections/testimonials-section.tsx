import type { TestimonialsContent, TestimonialsVariant, TestimonialItem } from "@/convex/db/storefronts/validators";
...
export function TestimonialsSection({ content, variant }: TestimonialsSectionProps) {
      ...
      <section className="py-20 px-4 md:px-8 bg-muted/30">
        <div className="max-w-7xl mx-auto space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {items.map((item) => (
              <div key={item.id} className="relative p-8 bg-background rounded-lg shadow-sm">
            ...
            {items.slice(0, 1).map((item) => (
              <div key={item.id} className="text-center space-y-6">
                <Avatar className="w-20 h-20 mx-auto">
                  <AvatarImage src={item.avatarUrl} />
                  <AvatarFallback className="text-2xl">{item.name[0]}</AvatarFallback>
                </Avatar>
